/* Auto-generated from pc_banner.txt
   - Pure JS module: injects CSS + HTML + runs original JS
   - Safe to import/execute once
*/
(function(){
  if (typeof window === 'undefined' || typeof document === 'undefined') return;
  if (window.__SDM_PC_BANNER_LOADED__) return;
  window.__SDM_PC_BANNER_LOADED__ = true;
...
if(cancelBtn){
  ...
  // 1) inject CSS (once)
  try {
    var st = document.getElementById('sdmPcBannerStyle');
    if (!st) {
      st = document.createElement('style');
      st.id = 'sdmPcBannerStyle';
  ...
  try {
    ...
    // prevent double-inserting the same markup
    if (!mount.__sdmInjected) {
      mount.innerHTML = HTML;
      mount.__sdmInjected = true;
    }
  } catch(e) {}

  // 3) run original JS after DOM nodes exist
  try {
    var sc = document.createElement('script');
    sc.id = 'sdmPcBannerScript';
